...
const BrandPage: React.FC = () => {
    ...
    const { loadingModels, models, error } = useSelector((state: { filter: FilterState }) => state.filter); 

    useEffect(() => {
        const args = { makeId: Number(makeId), year: Number(year) , MakeId: Number(makeId), MakeName: makeName};
        dispatch(fetchVehicleModels(args)); 
